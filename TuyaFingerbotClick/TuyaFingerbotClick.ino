/*
  TuyaFingerbotClick
  -------------------
  Etapa: implementar o protocolo Tuya BLE na ESP32 pra conectar no Fingerbot
  (CUBETOUCH II) via Bluetooth e mandar o comando de "click".

  O que é o protocolo Tuya BLE, resumido:
  - O Fingerbot não aceita "escrever 1 byte e pronto" via Bluetooth. Ele exige
    um handshake (troca de mensagens de apresentação) e depois só aceita
    comandos criptografados com AES.
  - Sequência: (1) ESP32 pede "informações do dispositivo" -> (2) dispositivo
    responde com um número aleatório (srand) usado pra derivar a chave de
    sessão -> (3) ESP32 manda "pair" (autenticação, usando device_id/uuid/
    local_key) -> (4) dispositivo confirma -> (5) ESP32 manda o comando real
    (DPS = "Data Points", os "campos" do dispositivo: DP2=mode, DP3=
    click_sustain_time etc).
  - Cada mensagem BLE só carrega ~20 bytes por vez (limite do "MTU" do
    Bluetooth Low Energy), então mensagens maiores são fatiadas em vários
    pacotes menores.

  GATT = o "dicionário" de características (characteristics) que um
  dispositivo BLE expõe. O Fingerbot expõe duas:
    - 0x2b11: onde a ESP32 ESCREVE os comandos.
    - 0x2b10: onde a ESP32 RECEBE notificações (respostas do dispositivo).
  Essas duas vivem dentro do "serviço" (service) 0xa201.

  Bibliotecas usadas (já vêm com o core ESP32 do Arduino IDE, não precisa
  instalar nada):
    - BLEDevice.h  -> Bluetooth Low Energy
    - MD5Builder.h -> hash MD5 (usado pra derivar as chaves de criptografia)
    - mbedtls/aes.h -> criptografia AES (usado pra empacotar os comandos)
*/
...
// Credenciais do Fingerbot ficam em secrets.h, que NÃO entra no Git (veja
// .gitignore). Copie secrets.h.example pra secrets.h e preencha antes de compilar.
#include "secrets.h"

// UUIDs do GATT (fixos, todo dispositivo Tuya BLE usa esses)
static const char* SERVICE_UUID       = "0000a201-0000-1000-8000-00805f9b34fb";
...
// (mandar dado de dentro do callback de notificação pode travar a pilha BLE).
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Função principal: conecta no Fingerbot via Bluetooth e manda o clique.
...
bool fingerbotClick(uint8_t sustainSeconds = 2, uint32_t timeoutMs = 10000) {
  // Deriva as chaves fixas a partir do local_key (não muda entre chamadas)
  ...
  Serial.println("[Tuya] conectando no Fingerbot via BLE...");
  ...
  // O UUID de serviço "oficial" (0xa201) é só o anunciado no advertising;
  // nem todo firmware expõe as características dentro dele. Por isso
  // procuramos em TODOS os serviços do dispositivo até achar as duas.
  g_writeChar = nullptr;
  g_notifyChar = nullptr;
  std::map<std::string, BLERemoteService*>* services = g_client->getServices();
  Serial.printf("[Tuya] %d servico(s) BLE encontrado(s):\n", services->size());
...
// ---------------------------------------------------------------------------
// Teste isolado desta etapa: liga o Wi-Fi não é necessário aqui, só Bluetooth.
// Depois que isso funcionar, chamamos fingerbotClick() a partir do código que
// já faz o polling no Firebase, no lugar deste teste no setup().
// ---------------------------------------------------------------------------
void setup() {
  Serial.begin(115200);
  delay(1000);
...
void loop() {
  // nada aqui por enquanto - isto é só o teste da etapa do Bluetooth
}
